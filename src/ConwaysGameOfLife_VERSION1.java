...
public class ConwaysGameOfLife_VERSION1 extends JFrame implements ActionListener {
    ...
    private class GameBoard extends JPanel implements ComponentListener, MouseListener, MouseMotionListener, Runnable {
        ...
        @Override
        //The bug was caused by run calling itself, resolved by taking the recursive call out of the method and creating a new method
        //The next bug caused stop to not work as it was in a while(true) statement and didnt allow for the interrupt to be raised
        //Resolved by using a boolean that was set to false upon catching an interrupt.
        public void run()
        {
